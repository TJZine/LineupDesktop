...
test('production playback capability profile admits libmpv direct-play attempts without enabling conversion', () => {
  const profile = getProductionCapabilityProfile();

  assert.equal(profile.id, 'windows-native-production-libmpv');
  assert.deepEqual(profile.directPlayContainers, ['*']);
  assert.deepEqual(profile.directPlayVideoCodecs, ['*']);
  assert.deepEqual(profile.directPlayAudioCodecs, ['*']);
  assert.deepEqual(profile.subtitleDeliveryModes, ['embedded', 'none']);
  assert.equal(profile.headerAuthSetup, 'supported');
  assert.equal(profile.seek, 'supported');
  assert.equal(profile.audioTrackSwitching, 'supported');
  assert.equal(profile.subtitleTrackSwitching, 'supported');
  assert.equal(profile.hdr, 'supported');
  assert.equal(profile.dolbyVision, 'supported');
