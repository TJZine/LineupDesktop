...
test('GuideRuntime post-setup refresh loads the committed channel without persistence or tune side effects', async () => {
  const repository = new MockChannelRepository();
  repository.data.channels = [createChannelConfig('chan-1', 1, 'Channel 1')];
  repository.data.currentChannelId = 'chan-1';
  let currentIdWrites = 0;
  repository.saveCurrentChannelId = async () => { currentIdWrites += 1; };
  const plexAdapter = new MockPlexLibraryAdapter();
  plexAdapter.setLibraryItems('lib-1', [createLibraryItem(0)]);
  const scheduler = new ChannelScheduler({ clock: { now: () => 1000 } });
  let tuneNotifications = 0;
  const runtime = new GuideRuntime({
    repository: repository as unknown as ChannelRepository,
    plexLibraryAdapter: plexAdapter as unknown as PlexLibraryMinimalAdapter,
    activeChannelScheduler: scheduler,
    clock: { now: () => 1000 },
    onChannelTuned: () => { tuneNotifications += 1; },
  });

  assert.deepEqual(await runtime.refreshAfterChannelSetupCommit(), { kind: 'completed' });
  assert.equal(scheduler.getState().channelId, 'chan-1');
  assert.equal(currentIdWrites, 0);
  assert.equal(tuneNotifications, 0);
});

test('GuideRuntime post-setup refresh reports interruption before mutating scheduler state', async () => {
  const repository = new MockChannelRepository();
  repository.data.channels = [createChannelConfig('chan-1', 1, 'Channel 1')];
  repository.data.currentChannelId = 'chan-1';
  const scheduler = new ChannelScheduler({ clock: { now: () => 1000 } });
  ...
  const runtime = new GuideRuntime({
    repository: repository as unknown as ChannelRepository,
    plexLibraryAdapter: new MockPlexLibraryAdapter() as unknown as PlexLibraryMinimalAdapter,
    activeChannelScheduler: scheduler,
    clock: { now: () => 1000 },
  });
  assert.deepEqual(await runtime.refreshAfterChannelSetupCommit(controller.signal), {
