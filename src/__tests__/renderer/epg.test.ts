...
test('EPG projects scheduler rows, stable cell ids, slots, clipping, and details', () => {
  ...
  const state = createEpgState(source, 7, 'wide');
...
test('Guide time range projects exact detailed and wide slot counts and durations', () => {
  ...
  const wide = createEpgGuideView(createEpgState(source, 6, 'wide'), source);
  ...
  assert.equal(wide.windowEndMs - wide.windowStartMs, EPG_WIDE_WINDOW_DURATION_MS);
});

test('time-range transition preserves a selected program by recentering without changing generation or currentness', () => {
  const source = presentation();
  const selected = {
...
test('program span excludes programs outside the active window', () => {
  assert.equal(calculateProgramSpan(program('outside', -3, -2), BASE, BASE + EPG_WIDE_WINDOW_DURATION_MS), null);
  assert.deepEqual(calculateProgramSpan(program('clipped', -1, 2), BASE, BASE + EPG_WIDE_WINDOW_DURATION_MS), {
    columnStart: 1,
    columnSpan: 2,
  });
});
...
test('directional navigation uses adjacent programs and nearest overlap on adjacent channels', () => {
  const source = presentation();
  const initial = createEpgState(source, 0, 'wide');
...
test('presentation refresh preserves a valid identity and clears stale tune feedback', () => {
  const source = presentation();
  const state = { ...createEpgState(source, 0, 'wide'), tuneError: 'Unable to tune.' };
...
test('Guide state and projected cells stay renderer-safe', () => {
  ...
  assert.equal(containsPlexForbiddenRendererField(createEpgGuideView(createEpgState(source, 0, 'wide'), source)), false);
});
