...
import { CHANNEL_ERROR_MESSAGES, MAX_CHANNELS, MAX_CHANNEL_NUMBER, MIN_CHANNEL_NUMBER } from './constants.js';
import {
...
export class ChannelAuthoringService {
  ...
  private applyOptionalFields(channel: ChannelConfig, input: Partial<ChannelCreateInput>): void {
    ...
    if (input.hidden !== undefined) channel.hidden = input.hidden;
  ...
  private validateOptionalFields(
    ...
    if (input.hidden !== undefined) validated.hidden = this.requireBoolean(input.hidden, 'hidden');
