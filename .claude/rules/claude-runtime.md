...
- Roles: `worker`, `worker_luna`, `reviewer` are Claude subagents with the same names in `.claude/agents/`. `.codex/config.toml` and `.codex/agents/*.toml` (OpenAI models, sandbox modes) configure Codex only. For Claude, the agent file sets model, effort, and tools; role semantics stay as the repository describes them. `worker` is the stronger implementer (Opus); `worker_luna` is the cost-efficient bounded implementer (Sonnet).
