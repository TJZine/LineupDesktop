...
| src/preload/index.cts | 1827 | The sandbox-compatible preload entrypoint still owns the single `lineupDesktop` exposure and `ipcRenderer` calls, while ARCH-02 moved channel constants to `src/preload/channels.cts`, diagnostics guard families to `src/preload/diagnosticsBridgeGuards.cts`, and snapshot RPC result hardening to `src/preload/playerBridge.cts`; the 2026-06-12 growth is limited to wiring that focused bridge, HLG guard vocabulary, and player dispatch result validation. | Split additional bridge guard families before any next growth in validation families, new bridge namespaces, or additional renderer-safe RPC arguments. |
...
| src/renderer/index.ts | 583 | Main entrypoint of the renderer orchestration. Keeps route activation, action handling, gamepad, keyboard bindings, and lifecycle listeners in one place. The 2026-06-12 growth is limited to snapshot-scoped track-selection dispatch and redacted dispatch-failure diagnostics. | Extract gamepad, keyboard listeners, player dispatch helpers, or action routing helpers before adding more route families, playback command families, or onboarding steps. |
