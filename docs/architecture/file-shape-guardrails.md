...
| src/renderer/overlays.ts | 508 | RD-13 fake-backed overlay state keeps OSD, mini guide, channel number, badge, and playback options in one renderer-local module. | Split overlay state by overlay family before RD-15 integrates overlays over native video. |
