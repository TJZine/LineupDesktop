...
| src/preload/index.cts | 1015 | RD-17 Unit 4 keeps diagnostics bridge vocabulary and runtime guards in the sandbox-compatible preload entrypoint so the exact `lineupDesktop` exposure, channel constants, invoke methods, renderer payload validation, renderer-safe request ids/context values, and renderer-safe result guards remain parity-tested in one reviewed surface. | Extract stable guard families or introduce reviewed preload bundling before adding another bridge namespace, new diagnostics result family, arbitrary RPC, extra contextBridge exposure, or any bridge vocabulary growth not covered by the parity/shape harness. |
