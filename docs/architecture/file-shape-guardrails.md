...
| src/preload/index.cts | 2045 | RD-22 Unit 1 keeps the exact Plex bridge vocabulary, request-id creation, renderer-input validation, and result validation in the sandbox-compatible preload entrypoint because plain `tsc` plus sandboxed preload still has no reviewed split/bundling path. This is a narrow same-file exception on top of the existing shell/window/player/diagnostics bridge guards. | Introduce reviewed preload bundling or another reviewed split before any later bridge namespace, broad guard family, arbitrary RPC, extra contextBridge exposure, or Plex bridge behavior beyond the RD-22 approved vocabulary grows this file again. |
