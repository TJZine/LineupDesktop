...
| src/renderer/routeDom.ts | 565 | Route-level DOM orchestration currently carries a compact set of route-specific render branches and guide shell composition paths together while parity polish and static binding coverage are in one place for this milestone. Growth in Unit 2 is for expanded now playing, mini guide, and OSD bindings. | Extract guide-dedicated rendering or overlay-specific rendering into dedicated helpers before adding additional route families, guide interaction families, or further parity-driven template branches. |
| src/renderer/index.ts | 560 | Main entrypoint of the renderer orchestration. Keeps route activation, action handling, gamepad, keyboard bindings, and lifecycle listeners in one place. Growth in Unit 2/3 adds channel buffer commit scheduling, mute/volume dispatch, and profile PIN orchestration hook. | Extract gamepad, keyboard listeners, or action routing helpers before adding more route families or onboarding steps. |
