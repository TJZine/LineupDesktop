...
| src/renderer/overlayViewModels.ts | 507 | Playback options view models currently keep static mappings and snapshot-to-view-model conversion together in one place for presentation. | Split view-model helpers or track conversion logic before adding more overlay view models or growing option row properties. |
