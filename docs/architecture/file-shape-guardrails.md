...
| src/preload/index.cts | 581 | Sandboxed preload remains single-file-compatible and exposes only the single `lineupDesktop` bridge with approved `ipcRenderer` method/channel pairs; comment-only seam documentation clarifies event guarding and invoke-envelope expectations. | Replan before adding new bridge methods, new channels, arbitrary RPC, extra contextBridge exposure, preload bundling, or any bridge vocabulary growth not covered by the parity/shape harness. |
...
| src/main/player/nativePlayerHostProcess.ts | 505 | RD-07 native host process owns child lifecycle, command framing, failure normalization, and cleanup/reap behavior in the main/player boundary; comment-only seam documentation distinguishes process framing checks from adapter event validation. | Split framing or cleanup helpers before production native-helper protocol expands. |
