...
| --- | ---: | --- | --- |
| src/main/player/desktopPlayerAdapter.ts | 1368 | RD-07 adapter centralizes player command validation, fakeable native-host state, request custody, and safe diagnostics in one owner while the native-helper boundary is still settling; the 2026-06-11 growth is limited to duplicate in-flight request-id rejection at the adapter boundary. | Decompose before production native-helper playback adds new command families, helper capabilities, or renderer-facing diagnostics. |
...
| src/main/player/nativePlayerHostProcess.ts | 541 | RD-25 native player host process manages spawned process lifecycle, stdin/stdout/stderr stream handling, NDJSON framing, command executor timeouts, and host event validation; the 2026-06-11 growth is limited to defensive duplicate pending request-id rejection before host writes. | Split process spawn/IO stream management from host event validation/command execution before adding new helper communication channels, diagnostics extraction, or stream protocol modes. |
...
| src/renderer/index.ts | 724 | Renderer runtime bootstrap and action/event orchestration is currently concentrated here while renderer shell and playback bootstrap alignment are still being stabilized. The 2026-06-11 growth covers guide/player bridge rejection state handling while shared renderer bridge failure diagnostics were extracted to `src/renderer/rendererBridgeFailures.ts`. | Decompose bootstrap initialization, runtime composition wiring, and action/event registration before adding additional renderer orchestration families or extra feature startup sequences. |
