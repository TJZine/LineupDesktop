...
| src/renderer/index.ts | 698 | Renderer runtime bootstrap and action/event orchestration is currently concentrated here while renderer shell and playback bootstrap alignment are still being stabilized. The 2026-06-11 growth is limited to refreshing guide presentation data when EPG paging changes the requested window. | Decompose bootstrap initialization, runtime composition wiring, and action/event registration before adding additional renderer orchestration families or extra feature startup sequences. |
