...
| RD20-M09 | RD-13 renderer UI, navigation, settings, guide, and player overlays | reference-only for RD-13; later setup-only adaptation separately ledgered | `src/modules/ui/**`, `src/core/channel-setup/**`, `src/styles/**`, `src/types/channelSwitch.ts` | RD-13 reference pin `76bc7ba31fa695ecef88b4ae79d40e8d79b7605f`, 2026-05-11; later Package 3 setup adaptation pin `cbdeaf57b3f59e52330e843005fcf02b3fbd586d`, 2026-07-13 | `src/renderer/**`, `src/__tests__/renderer/**`, `docs/architecture/renderer-architecture.md`, `docs/architecture/CURRENT_STATE.md`, `docs/architecture/import-ledger.md` | Package 3 setup-only behavior/DOM/CSS row in `docs/architecture/import-ledger.md` | RD20-D09 | compatible with qualified divergence | Source audit plus renderer navigation, workflow, setup-runtime, Plex-row, delegated-action, focus, route DOM, and style tests; Package 3 exact 36-capture proof; closeout commands `npm run verify:docs`, `npm run verify:redaction`, `npm run verify` | RD-13 itself used the scoped upstream UI/navigation/assets only as reference and landed no copied/adapted source. The later reopened Package 3 deliberately adapts only the exact ledgered channel-setup behavior/DOM/CSS sources into unprivileged renderer owners; browser runtime, credentials, persistence, Plex transport, scheduler, and unsupported strategy ownership remain excluded. |
