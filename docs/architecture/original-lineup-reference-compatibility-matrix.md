...
Packages 0–8 renderer parity and local automated verification are complete.
RD-20 remains historical docs/source-audit/provenance authority, extended by
the later reviewed Package 3 and Package 7 copied/adapted rows; Windows
operational parity remains pending in RD-27.
...
is the tracked compatibility artifact for M01-M07, M12, and M13 copied/adapted
import and ledger coverage, M08-M10 reference-only Desktop behavior, and M11
Desktop proof context. Compatibility rows are closeout evidence only for the reviewed scope;
...
| RD20-M12 | Reopened Package 3 channel-setup behavior, DOM, and CSS | copied/adapted | `src/modules/ui/common/ScreenShellView.ts`, `src/modules/ui/channel-setup/ChannelSetupScreen.ts`, `src/modules/ui/channel-setup/steps/LibraryStepController.ts`, `src/modules/ui/channel-setup/steps/LibraryStepPresenter.ts`, `src/modules/ui/channel-setup/steps/StrategyStepController.ts`, `src/modules/ui/channel-setup/steps/BuildReviewStepController.ts`, `src/modules/ui/channel-setup/steps/BuildProgressStepController.ts`, `src/modules/ui/channel-setup/styles.core.css`, `src/modules/ui/channel-setup/styles.library.css`, `src/modules/ui/channel-setup/styles.strategy.css`, `src/modules/ui/channel-setup/styles.review-progress.css`, `src/styles/shell.onboarding.shared-shell.css`, `src/styles/shell.onboarding.setup.css` | `cbdeaf57b3f59e52330e843005fcf02b3fbd586d`, 2026-07-13 | `src/renderer/staticDom.ts`, `src/renderer/setup/**`, `src/renderer/channelSetup/**`, `src/renderer/plexRuntimeRows.ts`, `src/renderer/plexRuntimeDom.ts`, `src/renderer/settingsSetup.ts`, `src/renderer/workflow.ts`, `src/renderer/domBindings.ts`, `src/renderer/rendererActionRegistration.ts`, `src/renderer/index.ts`, `src/renderer/styles/setup-workflow.css`, `src/renderer/styles/custom-channels.css`, focused renderer tests | `docs/architecture/import-ledger.md` Package 3 setup-only behavior/DOM/CSS row | RD20-D09 | compatible with divergence | Package 3 focused renderer suite, exact 36-capture proof, `npm run verify:docs`, `npm run verify:redaction`, and `npm run verify` | The adaptation is intentionally limited to the ledgered setup presentation and interaction slice in unprivileged renderer owners. Browser runtime, credentials, persistence, direct Plex transport, scheduler ownership, unsupported strategy families, and cross-process build cancellation remain excluded. |
| RD20-M13 | Package 7 Player overlay presentation hierarchy, density, motion, and accessibility | copied/adapted | Exact `src/modules/ui/player-osd/**`, `src/modules/ui/now-playing-info/**`, `src/modules/ui/mini-guide/**`, `src/modules/ui/channel-badge/**`, `src/modules/ui/channel-number-overlay/**`, `src/modules/ui/channel-transition/**`, `src/modules/ui/playback-options/**`, `src/modules/ui/common/OverlayPrimitives.ts`, and `src/modules/ui/types/overlay-primitives.ts` inputs enumerated by the Package 7 import-ledger row | pinned `4bdb0e1b3370e7893a582ec80226557727832d0b`, observed unchanged at `a1a7ea7dcb1cfc8aee7cfcf88cf5a1dac718bf30`, 2026-07-15 | `src/renderer/playerOverlayDom.ts`, `src/renderer/staticDom.ts`, `src/renderer/routeDom.ts`, `src/renderer/styles.css`, `src/renderer/styles/player-overlays.css`, `src/renderer/styles/player-overlay-information.css`, `src/renderer/styles/player-overlay-menus.css`, `src/renderer/styles/shell.css`, `src/renderer/styles/responsive-accessibility.css`, and focused renderer tests | `docs/architecture/import-ledger.md` Package 7 Player overlay presentation row | RD20-D12 | compatible with divergence | Package 7 focused 52/52 suite, 44 screenshots, 8 reduced-motion rows, 12 forced-colors rows, 3 local fullscreen rows, integrated Package 8 proof, `npm run verify`, and clean implementation review | Desktop adapts upstream presentation through renderer-safe view models while omitting artwork, unsupported controls, browser/webOS runtime, privileged playback data, and direct source/test/asset sharing. The linked Info-precedence divergence remains intentional; Windows operational proof is still required in RD-27. |
