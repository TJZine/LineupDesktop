...
remains complete only as historical divergence/provenance classification; its
rows do not waive the active plan's exact visual, interaction, runtime-truth,
or reviewed Desktop-divergence gates.
...
| RD20-D09 | RD20-M09 | RD-13 renderer UI, navigation, settings, guide, and player overlays | Renderer UI remains Desktop-owned and unprivileged. RD-13 was reference-only; the later reopened Package 3 setup surface is a separately ledgered focused adaptation. | RD-13 references under `src/modules/ui/**`, `src/core/channel-setup/**`, `src/styles/**`, and `src/types/channelSwitch.ts` at `76bc7ba31fa695ecef88b4ae79d40e8d79b7605f`; exact Package 3 channel-setup/ScreenShell/style sources at `cbdeaf57b3f59e52330e843005fcf02b3fbd586d` | Desktop still replaces upstream browser/runtime, settings persistence, Plex transport, guide refresh, and overlay coupling with local safe owners. Package 3 now materially adapts only the ledgered setup hierarchy, density, focus/multi-selection behavior, preview/review/progress structure, CSS expression, and inline confirmation while omitting unsupported strategies. | Desktop renderer must stay unprivileged and cannot own Plex transport, persisted secrets/settings, native handles, raw diagnostics, or live playback setup. The setup adaptation may use only existing renderer-safe callbacks and commit contracts; the custom-channel branch must remain labeled as Desktop-additive. | `src/renderer/**`, `docs/architecture/renderer-architecture.md`, `docs/architecture/CURRENT_STATE.md`, `docs/architecture/import-ledger.md` | Exact Package 3 renderer suite, 36-capture visual proof, source audit, redaction, full verification, and adversarial review | Preserves upstream setup structure and interaction fidelity for the Windows port without importing browser session/runtime assumptions or fabricating unsupported controls; the accepted qualification is limited to the separately ledgered Package 3 setup adaptation. | Package 3 focused/full automated proof plus controller visual inspection and `npm run verify:redaction` | accepted with follow-up | Replan if a change requires new preload/main contracts, live Plex renderer access, persisted settings IPC, browser storage, genuine unsupported strategy behavior, cross-process cancellation, or materially broader copied/adapted UI source. |
