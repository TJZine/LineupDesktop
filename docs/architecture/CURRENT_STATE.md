...
> 5E at `154fcfd`, Unit 5F at `3501fb8`, and Unit 5G at `4946fb5`; Unit 5H is
...
keyboard/D-pad focus reveal. Unit 5H is next. Windows composition and
...
| Main Guide presentation runtime | `src/main/channel/guideRuntime.ts`, `src/main/channel/guideArtworkOwner.ts`, `src/main/channel/desktopGuidePreferencesStore.ts`, `src/main/plex/guideArtworkSessionGenerationOwner.ts`, `src/main/plex/livePlexTransport.ts`, `src/main/protocol.ts`, and channel/Guide composition and IPC owners | Main-owned WS5 Units 5A–5B runtime for bounded scheduler-backed pages, public-library filtering, versioned filter persistence, opaque expiring artwork references, immutable Plex artwork sessions, and the closed `lineup://shell/artwork/` protocol. Raw Plex locators, bearer material, selected connections, and filesystem paths remain outside preload/renderer custody. |
