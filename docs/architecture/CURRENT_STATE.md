...
| Desktop persistence boundary | `src/main/persistence/appDataPaths.ts`, `src/main/persistence/secureStorageCodec.ts`, `src/main/persistence/desktopPersistenceStore.ts`, `src/main/persistence/desktopSettingsStore.ts`, `src/main/persistence/desktopChannelPersistenceStore.ts`, `src/main/persistence/channelPersistenceBootstrapOwner.ts`, `src/main/persistence/channelPersistenceStartupOwner.ts`, and `src/main/channel/desktopGuidePreferencesStore.ts` | Main-owned app-data paths, Electron safeStorage codec, encrypted Plex credentials, selected-server state, fail-closed recovery, version-2 normalized Settings compare-and-swap/migration, versioned channel aggregate persistence/startup, and WS5 versioned non-secret Guide library-filter preferences with atomic replacement, compare-and-swap, scope isolation, and fixed failures. Renderer receives only renderer-safe values, revisions, and fixed errors. Backup/restore remains unimplemented. |
