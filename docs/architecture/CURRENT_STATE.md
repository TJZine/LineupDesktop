...
> 5E at `154fcfd`, Unit 5F at `3501fb8`, Unit 5G at `4946fb5`, exact Guide
> presentation cancellation at `0ebaf2f`, global Now Watching projection at
> `8c30b1b`, and its captured-time/generation-currentness correction at
> `cf38e70`; Unit 5H is the next authorized package.
> Windows composition and large-live-lineup proof remains deferred to 5H, so
> Units 5D–5G do not close `EPG-10`–`EPG-13`, `UI-36`, or WS5.
...
WS5's reviewed plan is frozen at `73ce570`. Unit 5A secure Guide artwork and
play-to-now, Unit 5B persisted library filters/paging, and Unit 5C Now Watching
preference behavior land at `46acf1f`, `6180815`, and `beeb5ef`. Production
Unit 5D lands at `81cf42c` with the exact renderer/preload/main presentation
contract, bounded currentness queues, hide-before-load/switch/cleanup barriers,
and Player/Overlay/Classic Guide composition. The current production correction
uses an unowned helper-created disabled/no-activate popup positioned behind the
transparent, frameless Electron shell using its validated HWND as a reference,
then passes the popup HWND to libmpv through `wid`. Production sets
`vo=gpu-next`, `gpu-api=auto`, and `hwdec=auto` without a custom WGL/render-API
loop. Unit 5E `154fcfd` adds exact four-slot/two-hour Detailed and
...
`4946fb5` with fair main caps, bounded LRU retention, viewport/focus-aware DOM
...
generation. A bounded 2026-08-12 Windows production replay observed real Plex
Direct Play video, OSD and full Guide Overlay HTML above continuing video,
retained Electron focus, replacement tune playback without a black screen, and
a settled renderer. This is viability evidence, not durable/full closeout.
Broader Windows composition and large-live-lineup proof remains
deferred to 5H, so `EPG-10`–`EPG-13`,
`UI-36`, affected parity rows, and WS5 remain open. WS6–WS9 remain unopened.
...
`Escape` input, intentionally ignores `browser-forward`, forwards valid media
accelerators through Electron input, and forwards distinct play/pause/seek
actions through a closed semantic preload event. Raw app-command strings never
cross into the renderer. Unit 4 added renderer-owned DOM cursor
...
| Preload bridge | `src/preload/index.cts`, `src/preload/channels.cts`, `src/preload/guideBridge.cts`, `src/preload/settingsBridge.cts`, `src/preload/settingsBridgeGuards.cts`, `src/preload/playerRecoveryBridge.cts`, `src/preload/channelSetupBridge.cts`, `src/preload/channelBridgeGuards.cts`, and `src/preload/diagnosticsBridgeGuards.cts` | Narrow shell/window/player/diagnostics/Plex/channel/Settings bridge with runtime payload guards. WS3 exposes three Settings operations and guarded play/pause; WS4 adds the closed stop/relative-seek intents, required safe `seekSupport`, and a four-value main-to-renderer semantic media-input event. Guide presentation exposes only exact guarded get/cancel/filter operations; preload retains its active presentation request ids for cancellation correlation and exposes neither raw IPC strings nor `AbortSignal`. Snapshot identity is stripped before host dispatch, raw app-command strings are not exposed, and the sandbox-compatible entrypoint remains the sole `contextBridge`/`ipcRenderer` value owner. |
| Renderer shell, Settings, Channel Builder, input, and overlays | [`docs/architecture/renderer-architecture.md`](./renderer-architecture.md), `src/renderer/index.ts`, `src/renderer/settings/**`, `src/renderer/settingsSetup.ts`, `src/renderer/player/nativePlayerPresentationController.ts`, `src/renderer/playerErrorRecoveryController.ts`, `src/renderer/playerInputCommandController.ts`, `src/renderer/sleepTimerController.ts`, `src/renderer/desktopInput.ts`, `src/renderer/shell/navigationLifecycle.ts`, `src/renderer/channelSetup/builderConfigState.ts`, `src/renderer/workflow.ts`, and overlay/DOM/style owners | Unprivileged runtime-backed shell with WS1 Channel Builder, current version-3 Settings, scheduler-backed Guide, and safe Player/overlay projection. Guide G1 separates time range, performance profile, and row-density selection through one focused settlement owner; Auto and Reduced share foreground reach, issue no speculative idle requests, and use different bounded retention limits. Guide programs carry the exact renderer-safe nullable `poster`/`background` artwork set; G3B projects the selected valid background through the same-origin opaque artwork protocol with an independent stale-safe lifecycle, then issues a presentation-generation/reference/source/current-request-guarded opaque poster request when needed and terminates at the theme fallback if that poster request fails. The general artwork contract retains its non-Guide clear-logo capability. G3C now re-expresses the selected-detail hierarchy, Classic/Overlay shell visibility, one current-channel rail with one real `LIVE` status, 30-minute ruler/current marker, temporal cell states, All/per-library tabs, Now Watching, explicit loading/empty/error/retry states, and focus/typography/spacing/color roles in the existing renderer owners. G4 now provides pure 108px Comfortable/72px Compact row policy, Auto complete-row floors, measured complete-row viewport projection, bounded overscan, density/resize/DPI recomputation, and compact metadata/accessibility treatment without changing schedule/cache identity or density-only refresh behavior. WS4 adds semantic input aliases, context paging, source-aware 500 ms Back hold, serialized guarded direct commands, Subtitles/Sleep/Audio OSD, and session sleep-timer lifecycle. WS5 Units 5A–5D add focused artwork/detail rendering, Play-to-now, library tabs, focus-safe paging, Settings-controlled layout-specific Now Watching, and a two-phase native presentation aperture for Player/full Guide Overlay/playing-only Classic PIP without renderer access to Plex locators, native handles, or filesystem paths. Main remains authoritative; `index.ts` is composition wiring, and scroll-originated `guide-visible-window` queued, accepted, failure, and settled updates remain Guide-scoped rather than invoking unrelated application or native-presentation owners. G6 Windows/native/device/package evidence remains deferred. |
| Shell contract vocabulary | `src/contracts/shell.ts` | Renderer-safe shell/window/player bridge contract |
| Settings contract vocabulary | `src/contracts/settings.ts` and `src/contracts/settingsAudioValidation.ts` | Current exact version-3 renderer-safe values, defaults, normalization, capability projection, opaque audio-output rows, centralized strict audio-list/label validation, three-operation request/result vocabulary, and fixed safe failures. Guide time range, performance profile, and row density are independent; no legacy Settings key, version-specific migration, compatibility reader, or dual writer remains. Conservative native capabilities remain authoritative until later reviewed live proof. |
| Main Settings policy and audio owner | `src/main/settings/desktopSettingsSnapshotOwner.ts`, `src/main/settings/desktopSettingsPolicy.ts`, `src/main/settings/settingsAudioOutputOwner.ts`, `src/main/settings/settingsNativeHostComposition.ts`, and `src/main/settings/settingsIpc.ts` | Main-owned observation of the last successfully loaded or replaced renderer-safe Settings snapshot, authorized IPC, stream/resolver/diagnostic/private-helper preference consumers, opaque audio-device identity, and raw native-key custody. Startup performs one durable load for observed-snapshot and native-policy hydration. Explicit IPC loads still delegate to the fresh-reading durable store, successful replacements update the observation before returning, and Guide currentness checks synchronously project defensive in-memory copies without filesystem reads. The injectable composition factory creates the production host once and shares the same host binding with the audio owner and player IPC; development/smoke do not invoke the production factory. Renderer never receives raw device keys or promotes capability support. |
...
| Main Guide presentation runtime | `src/main/channel/guideRuntime.ts`, `src/main/channel/channelIpc.ts`, `src/main/channel/guideArtworkOwner.ts`, `src/main/channel/desktopGuidePreferencesStore.ts`, `src/main/plex/guideArtworkSessionGenerationOwner.ts`, `src/main/plex/livePlexTransport.ts`, `src/main/protocol.ts`, and channel/Guide composition and IPC owners | Main-owned WS5 runtime for bounded scheduler-backed pages, public-library filtering, versioned filter persistence, opaque expiring artwork references, immutable Plex artwork sessions, and the closed `lineup://shell/artwork/` protocol. Guide artwork authorization is reused only by current artwork-session generation, lineup revision, role, and normalized locator; active reissue refreshes its full 15-minute expiry in place while retaining the same authorization identity and ref id. Poster admits only numeric Plex thumb locators and background only numeric art locators. The exact `guide.cancelPresentation` operation authorizes the sender, owns request-scoped abort controllers and timeout/settlement/window-teardown cleanup in main, and propagates cancellation through Guide resolution without exposing a generic RPC seam. Global Now Watching is projected independently of the visible page/filter from the active scheduler only when its channel agrees with the committed generation; the program is recalculated at one captured time from the already-owned schedule/public references, with scheduler-item fallback and no off-page Plex resolution or Guide DTO expansion. Raw Plex locators, bearer material, selected connections, and filesystem paths remain outside preload/renderer custody. |
...
| Desktop persistence boundary | `src/main/persistence/appDataPaths.ts`, `src/main/persistence/secureStorageCodec.ts`, `src/main/persistence/desktopPersistenceStore.ts`, `src/main/persistence/desktopSettingsStore.ts`, `src/main/settings/desktopSettingsSnapshotOwner.ts`, `src/main/persistence/desktopChannelPersistenceStore.ts`, `src/main/persistence/channelPersistenceBootstrapOwner.ts`, `src/main/persistence/channelPersistenceStartupOwner.ts`, and `src/main/channel/desktopGuidePreferencesStore.ts` | Main-owned app-data paths, Electron safeStorage codec, encrypted Plex credentials, selected-server state, fail-closed recovery, current-only version-3 Settings compare-and-swap and revision-zero corrupt repair, a non-durable last-successful Settings observation, versioned channel aggregate persistence/startup, and WS5 versioned non-secret Guide library-filter preferences with atomic replacement, compare-and-swap, scope isolation, and fixed failures. `DesktopSettingsStore` retains fresh-read and serialized CAS ownership without a legacy migration/reader; Settings publication uses one exclusive same-directory private temp handle, complete write, mode enforcement, file synchronization, successful close, and atomic rename. Pre-rename failures preserve the destination and retain best-effort cleanup, while Windows rename power-loss durability remains an explicit release risk because no portable directory sync or Win32 write-through is owned. The observation owner adds no schema, path, or write algorithm. Renderer receives only renderer-safe values, revisions, and fixed errors. Backup/restore remains unimplemented. |
...
current-only version-3 main-owned Settings store, a focused last-successful snapshot
observation owner, a focused main policy and audio-output
...
path. One sleep-specific pause may wait behind play or relative seek and is
dispatched only after fresh same-request playing-state validation; all other
collisions fail closed, and a started pause is never retried. The timer owns no
persistence or power lifecycle. Conservative production capabilities and main/preload/native/Plex
...
Unit 5E at `154fcfd`, Unit 5F at `3501fb8`, and Unit 5G at `4946fb5`. Unit 5H
is next, while Windows composition and large-live-lineup proof remains deferred
there. WS6
