...
| Settings contract vocabulary | `src/contracts/settings.ts` | WS3 version-2 renderer-safe values, defaults, normalization, capability projection, opaque audio-output rows, three-operation request/result vocabulary, migration-compatible snapshots, and fixed safe failures. Conservative capabilities remain authoritative until later reviewed native/live proof. |
| Main Settings policy and audio owner | `src/main/settings/desktopSettingsPolicy.ts`, `src/main/settings/settingsAudioOutputOwner.ts`, and `src/main/settings/settingsIpc.ts` | Main-owned Settings snapshot projection, authorized IPC, stream/resolver/diagnostic/private-helper preference consumers, opaque audio-device identity, and raw native-key custody. Main shares one production helper host with player IPC; renderer never receives raw device keys or promotes capability support. |
