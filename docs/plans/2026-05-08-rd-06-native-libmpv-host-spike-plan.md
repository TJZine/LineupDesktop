...
app-owned presentation-boundary replan after `npm run verify:docs` passed.
...
**Windows helper-owned render API adjudication:** helper-owned render API is
blocked as the native surface strategy for RD-07 unless a later reviewed plan
reopens it with new evidence. The Windows render API smoke proved local dummy
media, dummy HTTP with only `X-Lineup-RD06: dummy`, windowed video pixels,
overlay pixels, focus, render API symbols, render context, render frame, helper
crash detection, cleanup, redaction, and libmpv API evidence. It did not capture
fullscreen video pixels through Electron capture or helper-owned scoped native
Win32 capture while `BrowserWindow` fullscreen was true. Composition remained
`not-proven-merged-capture-sources`, and render-thread discipline remained
`not-proven-blocking-helper-loop`. The practical conclusion is that this looks
like a real presentation/composition failure, not merely a Codex/browser capture
limitation.
...
failed fullscreen video-surface proof, and freeze one next bounded proof unit
before RD-07.
...
read-only review. It may add native-presentation-specific modes to
...
- Do not complete RD-06 from helper-owned render API evidence because the latest
  Windows smoke failed fullscreen, composition, and render-thread discipline
  proof.
...
- `docs/architecture/playback-architecture.md` records that the dev-only WID
  ...
  proof; this plan now records that helper-owned render API also failed the
  required fullscreen/composition/threading proof.
- `docs/roadmap/desktop-port-roadmap.md` records RD-06 as blocked/replan and
  ...
  this plan keeps RD-06 blocked until the app-owned native presentation boundary
  is proved or rejected.
...
- Helper-owned render API is no longer the chosen route for the next unit because
  the reviewed Windows smoke failed fullscreen native capture, composition, and
  render-thread-discipline proof even though render API frames were produced
  internally.
...
- Renderer owner: dev-only renderer content may provide a focus sentinel only;
  this unit does not attempt to prove DOM-over-native-video composition. The
  renderer must not receive secrets, raw URLs, raw headers, native handles,
...
- `git status --short --branch` observed branch `main...origin/main` with
  pre-existing unrelated dirty plan files. Those files must not be edited,
...
- This local checkout does not contain commits `ae41e7f` or `31ef560`; the
  supplied Windows handoff is treated as the authoritative render API evidence
  for this replan.
...
Observed Windows helper-owned render API proof-runner facts supplied to this
planner:
...
- Latest render API implementation added a render-API-only helper-owned
  Win32/GDI fallback gated on Electron-confirmed BrowserWindow fullscreen and
  scoped to the helper render child surface. Pass criteria require proven
  render-thread discipline and composition, so the failure cannot be waived by
  fullscreen pixels alone.
...
- The next unit should probe an app-owned native presentation boundary, not addon
  work or another helper-child-surface capture strategy, because the latest
  render API evidence produced internal frames but failed presentation,
  composition, and render-thread proof.
- If a native presentation boundary cannot be probed with the current dev helper
  and local Windows prerequisites, stop and replan. Do not smuggle
...
environment, or if the smoke fails fullscreen active video-surface, native
boundary composition, or render-thread-discipline proof, RD-06 remains
...
- Commit only after clean plan review, Windows native-presentation proof execution,
...
- If native presentation cannot be implemented or proved inside this scope, do not
