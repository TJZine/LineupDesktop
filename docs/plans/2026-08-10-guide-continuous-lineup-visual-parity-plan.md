...
`guidePresentationPolling.ts` remains the request/cancellation/currentness owner and consumes window intents. It retains one active foreground request plus one latest trailing foreground intent and issues no speculative idle requests. `src/renderer/index.ts` remains wiring-only: scroll/input callbacks delegate semantic intents and do not own paging, cache, or density policy.
...
`Reduced resource` does not change row density, time range, or channel availability. It uses the same foreground viewport request and DOM correctness, limits retained presentation pages/programs to the existing default bounds, and uses conservative artwork predecode/retention. `Auto` may use the existing upper bounds of 12 cached entries/12,000 programs. Neither profile issues speculative idle requests. Both profiles retain the existing main response caps of 200 programs per row and 1,000 total programs.
...
- no speculative idle request follows accepted foreground work;
...
- Default new-install Auto profile uses the complete viewport-derived row count and bounded PC retention policy; Reduced resource preserves identical correctness and visible-row density with lower retention limits.
