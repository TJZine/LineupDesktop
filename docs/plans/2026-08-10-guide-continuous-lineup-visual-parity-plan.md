...
- Do not add pre-MVP compatibility exports, dual old/new resolver methods, alias paths, deprecated proof schemas, or transition wrappers. Change each internal caller to the one current contract in the same G0 commit.
...
- `src/renderer/epg.ts`: >800-line Guide state/navigation projection; keep semantic EPG navigation cohesive, do not add request, DOM, persistence, or evidence-correlation policy, and require fresh architecture review. G0's fresh disposition is: Owner: `src/renderer/epg.ts`; Existing responsibility: pure EPG state, selection, and absolute Page-target projection; New behavior: synchronously report one accepted Page intent with the already computed absolute target before any consumer can queue work; Decision: cohesive growth; Evidence: acceptance and target derivation share the same navigation invariant, polling remains the request owner, `src/renderer/index.ts` remains wiring-only, no target math is duplicated, and the file is capped at 875 lines. A reviewer must confirm this disposition independently before G0 closeout.
...
- `src/domain/channel/channelManager.ts`: 1,022-line named hotspot and channel mutation/state owner. G0 changes only the existing `resolveFilteredItems` caller to destructure `.items` from the one classified `ContentResolver.resolveSource` result; all filtering, empty/error behavior, ordering, mutation, and state semantics remain byte-for-byte owned here and classification is ignored. Decision: cohesive caller adaptation with no proof policy, resolver policy, compatibility path, new helper, or line growth; hard cap <=1,022 and mandatory fresh architecture review.
- G0 uses materially reduced hard caps: `src/main/guideWindowsProofController.ts` <=425, `src/main/guideWindowsProofAnalysis.ts` <=350, `src/main/guideWindowsProofSampleAnalysis.ts` <=210, `src/main/guideWindowsProofEvidenceBinding.ts` <=300, `src/main/guideWindowsProofResourceSoak.ts` <=300, `src/main/channel/guideWindowsProofFixture.ts` <=100, `src/domain/channel/channelManager.ts` <=1,022, `src/renderer/guideProofResourceOwner.ts` <=200, `src/renderer/guideVirtualization.ts` <=275, `src/renderer/epg.ts` <=875, `src/contracts/guidePerformance.ts` <=180, `src/main/singleInstanceOwner.ts` <=150, `tools/guide-windows-proof.mjs` <=150, and `tools/guide-proof-artifact-publisher.mjs` <=160. `guideWindowsProofAnalysis.ts` is the sole public evidence-policy entry and composes the focused sample-analysis and evidence-binding implementation modules; the child controller and publisher both call that same entry. The controller owns orchestration/capture only, resource soak owns plateau/lifecycle analysis, the publisher owns canonical-root/file custody, hashes, redaction, no-overwrite, and atomic publication only, and the single-instance owner retains lock/secondary-notification policy. Do not evade a cap through compressed formatting, multiple semantic statements on one line, forwarding wrappers, mirrored state, or an unreviewed helper. Implementation review must inspect changed-owner line length and branch/correlation complexity in addition to the numeric caps; hitting a cap through dense formatting is a failure, not compliance.
...
G0's simplified reviewed selected boundary contains exactly these forty-seven paths:
...
- `src/main/guideWindowsProofEvidenceBinding.ts`
...
- `src/__tests__/main/guideWindowsProofEvidenceBinding.test.ts`
...
The first reviewed expansion from the approved thirty-three-path boundary was exactly `src/main/guideWindowsProofSampleAnalysis.ts`, `src/__tests__/main/guideWindowsProofSampleAnalysis.test.ts`, `src/main/guideWindowsProofResourceSoak.ts`, `src/__tests__/main/guideWindowsProofResourceSoak.test.ts`, `src/renderer/guideProofResourceOwner.ts`, `src/__tests__/renderer/guideProofResourceOwner.test.ts`, and `src/renderer/guideVirtualization.ts`. The accepted no-interruption amendment added exactly `src/main/singleInstanceOwner.ts` and `src/__tests__/main/singleInstanceOwner.test.ts`, taking the boundary from forty to forty-two. The simplified runtime-gate amendment adds exactly `src/renderer/epg.ts`, its existing focused owner `src/__tests__/renderer/epg.test.ts`, new `src/main/guideWindowsProofEvidenceBinding.ts`, and new `src/__tests__/main/guideWindowsProofEvidenceBinding.test.ts`, taking the retained boundary from forty-two to forty-six. The no-compat caller amendment adds exactly `src/domain/channel/channelManager.ts`, taking the retained boundary from forty-six to forty-seven; its existing focused `src/__tests__/domain/channelDomain.test.ts` owner was already selected. The rejected duplicate MJS validator and its structural test are deletion-only tombstones: removing those exact current G0-owned paths is authorized and required, but neither is part of the final retained selected boundary and neither may be replaced. All retained expansions, both tombstones, and the complete forty-seven-path boundary have zero overlap with all twenty-one `e7f1338` paths. Any other path is a collision/replan event rather than an implicit expansion.
...
Page acceptance uses one owner and one target calculation. `resolveEpgPageNavigation` in `src/renderer/epg.ts` retains the sole absolute Page-target calculation and accepts one exact synchronous `acceptPageIntent(targetGlobalIndex)` callback. For every handled Page decision, including an in-page or boundary-clamped decision, it invokes that callback exactly once with its already computed absolute target before returning the decision to `guidePresentationPolling.ts`; an unhandled decision invokes nothing. `src/renderer/index.ts` supplies only the callback that emits `input-accepted` with that target. For a fetch, polling records pending Page custody with that same absolute target before `queueRefresh` can emit request-start, loading state, or reconciliation; later attachment of the request sequence cannot change the target, and loading evidence cannot fall back to channel offset. Polling may cancel, hit cache, or queue the request only after acceptance. No caller recomputes or substitutes the target, and no promise, timer, microtask, duplicated mark, or post-request inference may repair ordering after the fact.

Evidence correlation is one canonical pure TypeScript analysis stack. `guideWindowsProofAnalysis.ts` exposes the sole internal analysis/publication entry, while `guideWindowsProofSampleAnalysis.ts` and `guideWindowsProofEvidenceBinding.ts` hold its focused implementation details and `guideWindowsProofResourceSoak.ts` retains the one resource/plateau implementation. The entry receives only fixed sanitized marks, chronological runtime slices, duration/frame records, resource identities, and fixed numeric navigation observations; composes exact no-reuse claims, strict main request/source arrival chronology, nonnegative duration validation, renderer/main request identity, cohort aggregation, resource plateau/exit validation, and first/middle/last accepted-presentation/focus settlement; and returns the one canonical summary/publication model. These owners have no Electron, filesystem, path, hashing, artifact-write, input-dispatch, or redaction custody. `guideWindowsProofController.ts` captures and orchestrates, then calls this entry. It does not retain parallel cohort bookkeeping or correlation policy.

Publication reuses the exact same built analyzer rather than a second validator. After the required Electron build, `tools/guide-proof-artifact-publisher.mjs` dynamically imports the canonical entry from `dist/main/guideWindowsProofAnalysis.js`, supplies each child's primary evidence plus the three-run set, and publishes only the analyzer-produced child and aggregate models. It never trusts or regenerates child conclusion fields. The analyzer binds proof mode/profile/eligible total/source cardinality to the fixed primary runtime/resource records and publisher invocation. The publisher independently recomputes each launcher-owned authority build hash and the exact child artifact hashes before writing the sole final manifest, and supplies the canonical distinct-root facts; arbitrary valid-looking authority is rejected. The publisher otherwise owns only containment, exclusive reads/writes, redaction scan, no-overwrite, manifest/replay custody, and atomic publication. It imports `validateGuideProofOutput` directly from `tools/smoke-electron.mjs`; no compatibility re-export remains. The built analyzer has no tool import and the publisher contains no sample, navigation, resource, percentile, schema-generation, or aggregation implementation.

Child provenance uses one launcher-owned file per disposable child. One fixed authority canonicalization/hash implementation lives in the publisher custody module and is called by both launcher preflight and publisher postflight; no second hash builder or schema exists. Immediately before spawn, after the exact Electron build is complete and after the empty-root/no-overwrite check, `tools/guide-windows-proof.mjs` uses that function and creates `authority.json` with `wx` exclusive semantics and exactly `{ schemaVersion: 1, authorityId, processRun, mode, profileMode, expectedEligibleTotal, headCommit, headTree, sourceStateSha256, buildSha256 }`. `processRun` is 1 for the single synthetic child and 1, 2, or 3 for live children; `mode` is `synthetic | live`; `profileMode` is `synthetic-auto | live-current`; the expected total is exactly 500 or the validated live total. `headCommit` and `headTree` are exact lowercase Git object ids. `sourceStateSha256` hashes one canonical byte stream containing those ids, `git diff --binary --no-ext-diff HEAD --`, and sorted repo-relative path/content hashes for untracked files in the exact G0 retained/tombstone set, so the current uncommitted proof implementation is covered without storing a local absolute path. `buildSha256` is one deterministic hash over every regular file anywhere under the freshly built canonical `dist` root, using sorted repo-relative path plus exact file bytes; this covers main, renderer, contracts, domain, preload, and copied assets. An empty build, a symlink, Windows reparse point, or any nonregular entry fails instead of being followed or omitted.

Before authority creation, the launcher reuses the existing bootstrap-root policy to resolve and verify canonical containment and reject reparse traversal. On every platform `wx` prevents replacement. On Windows the file inherits the already verified user-only bootstrap-root ACL/protection policy; the launcher re-verifies containment/no-reparse and does not claim POSIX mode bits. On POSIX it creates with mode 0600, applies `chmod(0600)`, and verifies the resulting mode. All authority hashes are lowercase 64-hex strings; no argv, local root, profile/server identity, channel/program text, or other private/free-form field is stored.

`authorityId` is the SHA-256 of the canonical remaining authority fields and therefore changes with the run identity or any source/build fact. In addition to the existing validated/contained output-root custody needed to write primary files, the launcher passes only `{ authorityId, processRun }` as provenance through the proof bootstrap capability. The child never reads or rewrites `authority.json` and echoes only those two values in the closed top level of its primary trace and summary; it never copies the output root or another provenance field into evidence. It writes no manifest or provenance conclusion.

After each child exits and before any publication, the publisher canonicalizes and proves all child roots distinct, reads the launcher-owned authority record, recomputes its `authorityId`, source state, and complete `dist` build hash using the same fixed rules and entry-type rejection, and requires exact pre/post equality. It then hashes the exact child `trace.json` and `summary.json` bytes, requires their echoed authority/run identities to match the root record, and rejects a swapped root, duplicate run, changed source/build, or post-exit artifact mutation. Only after all required children and the canonical analyzer pass does the publisher write the final no-overwrite `manifest.json` and `replay.txt`. The manifest contains no path: it records the common source/build authority plus per-run `{ authorityId, processRun, traceSha256, summarySha256 }`. Source/build drift during sampling fails closed with no final manifest or replay; ignored child files remain non-authoritative diagnostic residue.

Primary publication evidence is retained in closed chronological form. Sanitized `trace.json` contains marks and frames in stable ascending `timestampUs` order, with original capture ordinal as the tie-breaker, plus run/cohort/sample-keyed runtime slices whose exact existing fixed runtime events remain in controller arrival order. Every runtime slice is bounded to its actual request `channelLimit` and contains no channel/source identity or free-form text. Navigation observations retain only the fixed checkpoint kind, target/focus indices, connected-focus flag, offset/total/rows, and accepted-input, renderer-request, main-request, final-state, reconcile, and presented-frame identities needed for the canonical analyzer to bind them back to primary trace/runtime evidence. Resource records remain the exact fixed numeric schema already frozen above. Summary rows are derived conclusions, not substitutes for primary arrays.

### G0 simplification and no-compatibility contract

- Delete `tools/guide-proof-evidence-validator.mjs` and its test. No second evidence implementation, copied policy, compatibility export, or schema-shadow fixture replaces them.
- `GuideProofCohortRecord` retains only the run/cohort/sample-window identity and primary evidence required by the canonical analyzer. Remove unused `preparationInputs`, `dispatchedInputs`, and `runtimeEvidence`; derive counts from primary samples when required.
- Raw samples exist once under `runs[].rawSamples`. There is no duplicate top-level `rawSamples`; aggregation and publication flatten the run-owned arrays on demand.
- `guideWindowsProofResourceSoak.ts` is the sole process/resource projection and plateau owner. Delete duplicate `projectGuideResourceMetrics` logic and its structure-only test; the canonical analysis entry delegates resource validation to this owner.
- `ContentResolver.resolveSource` and `SourceResolutionCache.resolve` each become the one current classified-result operation. Delete `resolveSourceClassified` and `resolveClassified`, update all callers/tests atomically, and do not retain array-returning wrappers, aliases, or compatibility re-exports.
- `guideWindowsProofFixture.ts` owns one production synthetic fixture only: exactly 500 channels by 48 programs. Delete the exported variable-cardinality factory. The exact 459 regression uses a test-local deterministic no-network live-owner fixture and adds no production factory flexibility.
- `tools/guide-windows-proof.mjs` uses strict `node:util.parseArgs` string options with no positionals. Delete the hand tokenizer, duplicate-option state, and fallback spellings.
- Delete string-sniffing fake Electron/trace protocol engines and the publisher's hand-built shadow summary/trace/resource generator. Keep one real built-Electron synthetic smoke/assert, focused canonical analyzer/binder/resource tests, and the exact 459 selected-owner integration. Publisher tests receive one fixture produced by the canonical built analyzer.
- Delete tests that assert import names, line caps, helper presence, or implementation layout. Caps and complexity are independent review gates; automated tests protect behavior, containment, redaction, no-overwrite, chronology, navigation, resources, and publication output.

Adding the 1,022-line existing Channel Manager caller makes the comparable reviewed baseline 18,266 physical lines across forty-nine paths; its retained forty-seven paths currently contain 17,882. Before another runtime-approval review, both validator tombstones must be absent and the exact forty-seven retained files must total no more than 17,566 lines by the same `Get-Content` count: the two tombstones remove 384 lines and retained owners must still net at least 316 additional deletions, preserving at least 700 overall. Generated `dist`, this plan, ignored evidence, and unrelated files do not count. Joining statements, lengthening lines, deleting required counterexamples, or moving code outside the selected boundary does not satisfy the target; independent review inspects the actual diff for simpler ownership and preserved behavior.
...
Every run emits a complete cohort matrix with its one owned `rawSamples` array. Top-level summary contains `runs[]` and `aggregateByCohort[]` only; p50/p95/max are recomputed by flattening `runs[].rawSamples`, never averaged or derived from child percentiles. Each live child contains exactly one local run. The canonical built analyzer validates all three children, assigns top-level process-run identities 1 through 3, rejects missing/duplicate/mislabeled/undersized cohorts, creates the three-run aggregate, retains each child trace/summary under `runs/run-N`, and writes a top-level run-indexed trace record rather than presenting run 1 as the whole live trace.
...
### G0 accepted implementation/runtime-review corrections before rerun
...
- real Page handling emits its one `input-accepted` mark through the EPG-owned callback before synchronous request-start, loading state, or reconciliation. Its target is the exact global target returned by the sole EPG calculation. Direct EPG and polling integration counterexamples must fail any post-request mark, microtask repair, duplicate target calculation, duplicate acceptance, or substituted local target;
- `guidePerformanceMarks.ts` clears the matching active request immediately when settlement is rejected or aborted. Only an accepted request may remain active until its final accepted state and reconcile; a later Arrow/Page/pointer/gamepad input can never inherit a failed request sequence;
- strict main evidence uses controller arrival order: exactly one request-start precedes zero-to-that-request's-actual-`channelLimit` source-settled events, and every source settlement precedes the matching request-settled. A request with limit 12 rejects a thirteenth source settlement even though the global maximum is 24. Duplicate, foreign, out-of-order, post-settle, over-limit, or negative-duration records fail. A negative elapsed value in `guideRuntime.ts` or `guideWindowsProofResourceSoak.ts` invalidates proof observation without changing the product result; it is never emitted, stored, or analyzed as zero. The canonical analyzer rejects any negative record that reaches it;
- canonical sanitized marks, duration records, and frame arrays use stable chronological order before analysis or publication. Reverse-ordered and equal-timestamp inputs prove deterministic first-presentation selection and original-ordinal tie breaking;
- first/middle/last navigation checkpoints are final displayed-navigation proof, not request intent. Each binds an accepted Page target, accepted renderer request, final-data state, reconcile completion, first subsequent canonical presented frame, connected focused program at the intended global index, and the exact settled main request/window. The controller must leave offset zero and return through Page Up before recording `first`, so even the first checkpoint proves a completed navigation rather than inheriting startup state. Warm, poll, stale, failed, loading-only, disconnected-focus, request-start-only, or mismatched-window records fail;
- the canonical TypeScript analyzer receives correlation/no-reuse logic removed from the controller and all deep evidence policy removed from the publisher. The publisher dynamically imports that exact built entry and cannot accept a child merely because its self-reported conclusions are well shaped. There is no MJS validator, second schema, or duplicated aggregation;
- the canonical analyzer recomputes every sample and cohort from primary evidence, including input-received, input-accepted, request-start/settled, state acceptance, reconcile-start/end, main/source chronology and durations, layout/paint, long tasks, navigation/focus, all resource plateau/exit formulas, run-owned raw-sample statistics, three-run aggregation, source/build/profile/eligible-total consistency, and provenance. The publisher independently owns canonical-root/digest/redaction custody facts and supplies them to the same publication result. Missing primary arrays, inconsistent child conclusions, or child-authored pass flags fail closed; and
- exact live-mode regression uses expected total 459 and the full one-local-run live matrix through the real EPG Page intent, Guide polling, performance-mark, main Guide runtime, content-resolver/source-cache, resource-soak, sample-analysis, and controller owners. Deterministic no-network sources may replace external Plex transport, and Electron trace/native-input/process metrics may use bounded ports, but the test may not hand-emit the ideal Page/request/state/reconcile order. The existing synthetic 500-by-48 fixture remains separate and does not satisfy this live-owner test.

Acceptance:

- the exact forty-seven-path boundary remains unique and disjoint from `e7f1338`, both removed validator paths are absent, the selected-file total is <=17,566 lines without compressed formatting, every revised owner cap passes, and mandatory fresh architecture review accepts both the `epg.ts` and caller-only `channelManager.ts` dispositions above;
- direct Page integration proves `input-received` then the EPG-owned exact-global-target `input-accepted` before any request/loading/reconcile record, rejected/aborted request state cannot leak into a later input, and strict main chronology/negative-duration counterexamples fail;
- the exact 459 live-mode real-owner fixture and the separate fixed 500-by-48 synthetic fixture both complete their full applicable matrices, and the publisher's dynamically imported canonical analyzer reconstructs rather than trusts all published conclusions;
...
- both proof modes fail rather than overwrite an existing output directory. Each disposable child contains only launcher-owned `authority.json` and child-owned primary `summary.json`/`trace.json`; after exact post-run source/build/artifact recomputation and canonical analysis, the publisher writes final `manifest.json`, aggregate `summary.json`, run-indexed `trace.json`, and `replay.txt`. The manifest records mode, common source/build authority, fixture kind or live eligible total, environment, runs/samples, mark schema version, cache/poll cohorts, redaction status, and exact per-run authority/trace/summary hashes without a path. `replay.txt` contains the exact successful command with secrets and local user paths omitted;
...
- selected-boundary line audit using the exact retained list above — both validator tombstones are absent, all forty-seven paths are unique, total physical lines are <=17,566, and no changed owner shows cap-compressed formatting.
...
- `src/__tests__/renderer/epg.test.ts` plus `guidePresentationPolling.test.ts` prove exactly one EPG-computed absolute Page target is accepted synchronously before request/loading/reconcile work, including a handled boundary-clamped Page action, with no callback for an unhandled decision and counterexamples for post-request acceptance, duplicate acceptance, and local/global substitution; no microtask-order test is accepted as the positive contract;
- `guidePerformanceMarks.test.ts` proves rejected/aborted settlement clears active request immediately and cannot poison the next Arrow/Page/pointer/gamepad reconcile, while accepted loading/final settlement retains then clears the exact request;
- `guideWindowsProofEvidenceBinding.test.ts` directly proves strict main arrival chronology, bounded source settlement, nonnegative intervals, renderer/main window identity, final accepted navigation/presentation/connected-focus settlement, and shared no-reuse rejection. `guideWindowsProofSampleAnalysis.test.ts` retains cohort counts/statistics and proves sample-end-bounded canonical frame selection, rejection of extra inputs/states, exact accepted-input/request-origin/request-class binding, `number | 'not-applicable'` fields, cold-live loading/final schema, quiet-poll and poll-collision origin binding, settled and identity-separated source-cache preparation, authoritative proof-renderer-main layout/paint/long-task interval attribution, and `distanceToNextPollMs`;
- `guideWindowsProofAnalysis.test.ts` proves sanitized marks, duration records, and frames are stable chronological arrays, including reverse input and equal-timestamp capture-order counterexamples, and rejects invalid negative or reversed duration intervals;
- one real built-Electron synthetic smoke/assert proves the fixed 500-by-48 fixture launches through the validated capability, reaches the controller, produces canonical primary evidence, and preserves containment/no-overwrite/redaction. Focused controller tests use explicit typed ports only; delete every fake that branches on `executeJavaScript` source text or reimplements an Electron/trace engine;
- one exact live-mode expected-total-459 integration completes the applicable full matrix through the real selected EPG/polling/mark/main-runtime/content-resolver/source-cache owners with deterministic no-network ports, including Guide re-entry, boundary-clamped Page acceptance, final navigation/frame/connected-focus settlement, exact actual-limit source counts, and resource lifecycle. Hand-emitted ideal semantic order cannot satisfy it;
- domain/main tests consume only the one classified `ContentResolver.resolveSource` and `SourceResolutionCache.resolve` result, proving cold/shared/cache-hit classification and ordinary callers' item behavior after the atomic API change; `channelDomain.test.ts` also proves Channel Manager's existing resolved-item, filtering, and empty-source error behavior is unchanged when its sole caller destructures `.items` and ignores classification. No old method is exercised or exported;
- fixture tests prove production synthetic creation is exactly 500 channels by 48 programs and rejects configuration other than the fixed proof authority, while the 459 integration constructs its lineup inside the test;
- launcher tests exercise strict `node:util.parseArgs` handling for the closed options, unknown options, invalid values, and forbidden positionals without testing a hand-parser implementation;
...
- canonical TypeScript analyzer tests prove recomputation from primary marks/frames/runtime slices/resources/navigation observations, including input-received, reconcile-start, main/source chronology bounded by the actual request limit, layout/paint, long tasks, final navigation/focus, every plateau/exit formula, run-owned raw statistics, three-run aggregation, and provenance. They reject fabricated child conclusions, missing primary evidence, reverse chronology, negative duration, stale/disconnected navigation, and summary/reconstruction disagreement. Publisher tests use one canonical-analyzer-produced fixture and cover only three distinct child roots, containment/hashes/redaction/file custody, atomic no-overwrite publication, top-level run identities, and propagation of analyzer rejection;
- launcher/publisher custody tests prove the exact `authority.json` schema and `wx` exclusive write, Windows inherited user-only root protection plus containment/no-reparse checks, POSIX chmod/stat 0600 verification, deterministic authority-id recomputation, child echo of only authority/run identity, three distinct canonical roots/runs, pre/post source and complete-`dist` equality, exact trace/summary byte hashes, and path-free final manifest. Tests prove main, renderer, contracts, domain, preload, and asset bytes affect `buildSha256`; an empty tree, symlink/reparse/nonregular entry, tracked/untracked source mutation, build mutation, trace/summary mutation, swapped root/run, duplicate child, extra authority key, or final overwrite fails before manifest/replay publication;
...
Before review, inspect the changed controller, canonical analyzer stack, resource owner, publisher, resolver/cache owners, fixture, and launcher for overlong or multi-statement policy lines and branch/correlation complexity alongside `npm run verify:maintainability`. The numeric caps and >=700-line deletion are simplicity ceilings, not formatting targets; structural import-name/line-count tests are forbidden, and a reviewer finding that logic was compressed or moved to fit stops G0 even when counts pass.
...
- G0 proof requires any path outside its exact forty-seven retained paths plus the two named deletion-only tombstones, overlaps a concurrent writer or `e7f1338`, or exceeds a reviewed owner cap without a cohesive extraction;
- honest Page acceptance cannot be emitted synchronously from the EPG-owned absolute target before polling/request/loading/reconcile work without duplicating target math, adding asynchronous timing, or moving request policy into `epg.ts`;
- the canonical analyzer cannot be reused directly from `dist/main/guideWindowsProofAnalysis.js`, would require a second evidence implementation/schema, Electron/filesystem/renderer privilege, a dependency cycle, or duplicate correlation/aggregation policy in the controller or publisher;
- exact launcher-owned authority cannot cover HEAD/tree, tracked and selected-untracked source state, and every regular byte under fresh canonical `dist` with one path-free fixed record; authority creation cannot use `wx` plus the frozen Windows/POSIX protection checks; the child would need to read/trust that file or echo more than its authority/run hash reference; or the publisher cannot recompute pre/post authority and exact primary artifact hashes before final publication;
- source, tracked diff/tree, selected untracked input, or any canonical `dist` byte/entry changes between a child's authority write and post-exit verification; this is a failed run, never a provenance amendment or partial publication;
- strict primary evidence cannot retain chronological fixed runtime slices and navigation identities without private/free-form data, or publisher recomputation would need to trust child-derived conclusions;
- the exact 459 live-mode full-matrix test cannot exercise the selected real EPG/polling/mark/main-runtime/content-resolver/source-cache owners without external Plex/network access or hand-emitted semantic order;
- the two rejected validator paths remain, the exact forty-seven selected files exceed 17,566 lines, or a cap/deletion target can be met only through compressed formatting, deleted behavior gates, moved code, or unreviewable complexity;
- another production caller of the retired array-returning resolver/cache contract appears outside the exact forty-seven retained paths, or Channel Manager requires anything beyond destructuring `.items` while preserving its existing behavior and ignoring classification;
- removing the old resolver/cache methods would require a compatibility wrapper or leave a caller unable to consume the one current classified result;
- the fixed production 500-by-48 fixture cannot serve synthetic proof without a variable production factory, or the 459 test cannot remain test-local/deterministic;
- strict `node:util.parseArgs` cannot express the closed launcher options without a compatibility parser or fallback spelling;
