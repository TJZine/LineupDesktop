...
This is a separate durable Guide handoff and does not edit or supersede the user-modified `docs/plans/2026-07-22-tier3-parity-correction-plan.md`. Its active `WIN-TEST-006` amendment currently makes the playback correction and operator-assisted Windows proof the sole product-change exception. No Guide package, including G0 instrumentation, may begin until that proof passes or the amendment's current owner formally replans the sequencing. After that authority gate, the Guide controller must record the playback checkpoint, confirm that no selected Guide-package file has a concurrent writer, and inspect the worktree for collisions. A collision or materially changed Guide baseline stops the current package for replan; it is not permission to overwrite or absorb concurrent work.
...
BLOCKERS: `WIN-TEST-006` operator-assisted Windows proof must pass or its active amendment must be formally replanned; then confirm no selected-package files overlap
MESSAGE:
Follow the reviewed Tier 3 plan through the feature-quality loop. First enforce the `WIN-TEST-006` sequencing gate. After it passes or is formally replanned, execute G0 through G6 serially with a review/verification/commit checkpoint after each package while preserving unrelated playback and user edits. The required product outcome is continuous populated scrolling across all 459+ channels plus current-upstream visual parity and PC-specific row/resource policies; do not stop at the former twelve-channel presentation, synthetic proof, or a visual-only approximation.
