...
- G0 proof owners use these reviewed hard line caps: `src/main/guideWindowsProofController.ts` <=500, `src/main/guideWindowsProofAnalysis.ts` <=500, `src/main/guideWindowsProofSampleAnalysis.ts` <=400, new `src/main/guideWindowsProofEvidenceBinding.ts` <=350, `src/main/guideWindowsProofResourceSoak.ts` <=400, `src/renderer/guideProofResourceOwner.ts` <=250, `src/renderer/guideVirtualization.ts` <=275, `src/renderer/epg.ts` <=875, `src/contracts/guidePerformance.ts` <=180, `src/main/singleInstanceOwner.ts` <=150, `tools/guide-proof-artifact-publisher.mjs` <=200, and new `tools/guide-proof-evidence-validator.mjs` <=450. The controller remains orchestration/capture-only; the existing analyzer remains raw-trace sanitation/diagnostics plus delegation; sample analysis owns cohort requirements and aggregation; evidence binding owns pure chronological request, accepted-presentation/focus, duration, and no-reuse correlation; resource soak owns plateau/lifecycle analysis; the renderer resource owner owns fixed numeric observation; the publisher owns canonical-root/file custody and atomic publication only; and the pure tool validator independently reconstructs evidence authority. The single-instance owner retains lock/secondary-notification policy. Controller and sample analysis import evidence binding, never the reverse; the publisher imports the validator, never the reverse; the tool validator does not import or delegate to the product analyzer/binding. Do not evade a cap through compressed formatting, multiple semantic statements on one line, forwarding wrappers, mirrored state, or an unreviewed helper. Implementation review must inspect changed-owner line length and branch/correlation complexity in addition to the numeric caps; hitting a cap through dense formatting is a failure, not compliance.
...
G0's reviewed selected boundary contains exactly these forty-eight paths:
...
- `tools/guide-proof-evidence-validator.mjs`
...
- `tools/__tests__/guide-proof-evidence-validator.test.mjs`
...
The first reviewed expansion from the approved thirty-three-path boundary was exactly `src/main/guideWindowsProofSampleAnalysis.ts`, `src/__tests__/main/guideWindowsProofSampleAnalysis.test.ts`, `src/main/guideWindowsProofResourceSoak.ts`, `src/__tests__/main/guideWindowsProofResourceSoak.test.ts`, `src/renderer/guideProofResourceOwner.ts`, `src/__tests__/renderer/guideProofResourceOwner.test.ts`, and `src/renderer/guideVirtualization.ts`. The accepted no-interruption amendment added exactly `src/main/singleInstanceOwner.ts` and `src/__tests__/main/singleInstanceOwner.test.ts`, taking the boundary from forty to forty-two. The runtime-gate amendment adds exactly `src/renderer/epg.ts`, its existing focused owner `src/__tests__/renderer/epg.test.ts`, new `src/main/guideWindowsProofEvidenceBinding.ts`, new `src/__tests__/main/guideWindowsProofEvidenceBinding.test.ts`, new `tools/guide-proof-evidence-validator.mjs`, and new `tools/__tests__/guide-proof-evidence-validator.test.mjs`, taking the boundary from forty-two to forty-eight. All three expansions and the complete forty-eight-path boundary have zero overlap with all twenty-one `e7f1338` paths. Any further path is a collision/replan event rather than an implicit expansion.
...
Page acceptance uses one owner and one target calculation. `resolveEpgPageNavigation` in `src/renderer/epg.ts` retains the sole absolute Page-target calculation and accepts one exact synchronous `acceptPageIntent(targetGlobalIndex)` callback. For every handled Page decision, including an in-page or boundary-clamped decision, it invokes that callback exactly once with its already computed absolute target before returning the decision to `guidePresentationPolling.ts`; an unhandled decision invokes nothing. `src/renderer/index.ts` supplies only the callback that emits `input-accepted` with that target. Polling may cancel, load, reconcile, hit cache, or queue `requestPage` only after the callback has run. No caller recomputes or substitutes the target, and no promise, timer, microtask, duplicated mark, or post-request inference may repair ordering after the fact.

Evidence correlation is a distinct pure main owner. `guideWindowsProofEvidenceBinding.ts` receives only fixed sanitized marks, chronological runtime slices, duration/frame records, resource identities, and fixed numeric navigation observations. It owns exact no-reuse claims, strict main request/source arrival chronology, nonnegative duration validation, renderer/main request identity, and first/middle/last accepted-presentation/focus settlement. It has no Electron, filesystem, tracing, artifact, input-dispatch, cohort-count, percentile, or publication custody. `guideWindowsProofController.ts` captures and orchestrates; `guideWindowsProofSampleAnalysis.ts` owns cohort applicability/counts and statistics; both consume binding results and neither retains a second correlation implementation.

Publication validation is deliberately independent of product analysis. `tools/guide-proof-evidence-validator.mjs` is a pure closed-schema validator with no filesystem, process-launch, path, hashing, or write access. The publisher supplies parsed child manifests/summaries/traces plus publisher-computed canonical-root and artifact-digest facts. The validator returns one authoritative validated publication model only after independently reconstructing input-received/accepted, renderer request/state/reconcile-start/reconcile-end, strict main request/source settlement, layout/paint, long-task, canonical presentation, navigation/focus, resource plateau/cleanup, run/cohort aggregation, and provenance consistency from primary records. It compares all child conclusions with that reconstruction and rejects disagreement; it never trusts a child's booleans, percentiles, aggregates, navigation/resource verdicts, authority tuple, or provenance conclusion. The publisher owns distinct-root containment, exclusive reads/writes, digests, manifest/replay custody, and atomic publication only; it must not reimplement or selectively bypass evidence policy. The independent tool validator does not import or forward to `guideWindowsProofEvidenceBinding.ts` or either analyzer.

Primary publication evidence is retained in closed chronological form. Sanitized `trace.json` contains marks and frames in stable ascending `timestampUs` order, with original capture ordinal as the tie-breaker, plus run/cohort/sample-keyed runtime slices whose exact existing fixed runtime events remain in controller arrival order. Every runtime slice is bounded to its channel limit and contains no channel/source identity or free-form text. Navigation observations retain only the fixed checkpoint kind, target/focus indices, connected-focus flag, offset/total/rows, and accepted-input, renderer-request, main-request, final-state, reconcile, and presented-frame identities needed for the validator to bind them back to primary trace/runtime evidence. Resource records remain the exact fixed numeric schema already frozen above. Summary rows are derived conclusions, not substitutes for any primary array.
...
- strict main evidence uses controller arrival order: exactly one request-start precedes zero-to-channel-limit source-settled events, and every source settlement precedes the matching request-settled. Duplicate, foreign, out-of-order, post-settle, over-limit, or negative-duration records fail. A negative elapsed value in `guideRuntime.ts` or `guideWindowsProofResourceSoak.ts` invalidates proof observation without changing the product result; it is never emitted, stored, or analyzed as zero. Evidence binding and the independent publisher validator reject any negative record that reaches them;
...
- the new pure evidence-binding owner receives the correlation/no-reuse logic removed from the controller and sample analyzer; both existing owners must shrink to their new caps without retaining parallel binders. The new independent tool validator receives deep child-evidence validation removed from the publisher; the publisher must shrink to custody/publication and cannot accept a child merely because its self-reported conclusions are well shaped;
- the publisher validator recomputes every sample and cohort from primary evidence, including input-received, input-accepted, request-start/settled, state acceptance, reconcile-start/end, main/source chronology and durations, layout/paint, long tasks, navigation/focus, all resource plateau/exit formulas, raw-sample statistics, three-run aggregation, distinct-root/digest authority, source/build/profile/eligible-total consistency, and provenance. Missing primary arrays, inconsistent summary conclusions, or child-authored pass flags fail closed; and
...
- the exact forty-eight-path boundary remains unique and disjoint from `e7f1338`, every revised owner cap passes without compressed formatting, and the mandatory fresh `epg.ts` architecture review accepts the cohesive disposition above;
...
- the exact 459 live-mode real-owner fixture and the separate 500-channel synthetic fixture both complete their full applicable matrices, and the independent publication validator reconstructs rather than trusts all published conclusions;
...
- `src/__tests__/renderer/epg.test.ts` plus `guidePresentationPolling.test.ts` prove exactly one EPG-computed absolute Page target is accepted synchronously before request/loading/reconcile work, with no callback for an unhandled decision and counterexamples for post-request acceptance, duplicate acceptance, local/global substitution, and boundary clamping; no microtask-order test is accepted as the positive contract;
...
- G0 controller/orchestration tests complete the full matrix, re-enter Guide for runs two and three, and reject invalid cold, cache, poll, frame, route, navigation/focus, runtime chronology, and resource lifecycles. One exact live-mode expected-total-459 full-matrix fixture must run through the real selected EPG/polling/mark/main-runtime/content-resolver/source-cache owners with deterministic no-network ports; helper-only happy paths, hand-emitted ideal marks, or marks emitted while on another route are insufficient;
...
- `guide-proof-evidence-validator.test.mjs` proves independent recomputation from primary marks/frames/runtime slices/resources/navigation observations, including input-received, reconcile-start, main/source chronology, layout/paint, long tasks, final navigation/focus, every plateau/exit formula, raw statistics, three-run aggregation, and provenance. It rejects self-consistent fabricated child conclusions, missing primary evidence, reverse runtime chronology, negative duration, stale/disconnected navigation, repeated/aliased authority, and summary/reconstruction disagreement. Publisher tests retain three distinct complete live child roots, containment/digest/file custody, atomic publication, evidence preservation, top-level run identities, and rejection of a repeated root or validator failure; publisher fixtures may not replace the validator's primary-evidence counterexamples;
...
Before review, inspect the changed controller, sample analyzer, evidence binder, publisher, and validator for overlong or multi-statement policy lines and correlation branch density alongside `npm run verify:maintainability`. The numeric caps are ceilings, not formatting targets; a reviewer finding that logic was compressed to fit stops G0 even when line counts pass.
...
- G0 proof requires any path outside its exact forty-eight-path boundary, overlaps a concurrent writer or `e7f1338`, or exceeds a reviewed owner cap without a cohesive extraction;
...
- the evidence binder or tool validator would become a forwarding wrapper, require Electron/filesystem/renderer privilege, introduce a dependency cycle, or leave duplicate correlation/deep-validation policy in the controller, sample analyzer, or publisher;
...
- the controller, sample analyzer, publisher, EPG owner, evidence binder, or validator cannot meet its reviewed cap without compressed formatting or unreviewable correlation complexity;
