...
- `guidePerformanceMarks.test.ts` proves the exact seven names and closed primitive details, input/request sequence reuse only for the active semantic chain, rejected/aborted and final-reconcile cleanup, immediate `clearMarks`, absence of the removed custom frame/resource/provenance surfaces, and fail-open behavior when either `mark` or `clearMarks` throws;
