...
playback headers, and session cleanup through it. A recognized PMS authorization
...
- **Evidence:** distinct synthetic credentials cover discovery, probes,
  libraries, artwork, playback, release, refresh, cancellation, logout, and
...
- **Ownership:** Flutter coordinates ordered native loads, known-boundary seeks,
  stale-event rejection, credential replacement, and one aggregate release.
