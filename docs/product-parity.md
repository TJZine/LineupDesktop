...
| OSD hierarchy | Title/status, progress/buffer, up-next, tracks, sleep | At 1280×720 and 1920×1080, a responsive shallow bottom-edge gradient keeps restrained metadata in the lower-left/lower band, secondary actions in the lower-right, places the channel bug top-right, and anchors an edge-to-edge progress line to the absolute bottom; transport buttons are hidden by default and restored by the DVR setting | INTENTIONAL DESKTOP ADAPTATION | — | HIGH | `lib/playback/player_view.dart`; `test/playback/player_view_test.dart`; 1280×720 and 1920×1080 OSD goldens; screenshot `00-25-11` | Player UI |
...
All 20 current 1280×720 baselines have accepted macOS pixels, with five
additional focused 1920×1080 baselines for Mini Guide, OSD, Rich Now Playing,
Profiles, and Channel Setup review. The principal remaining responsive risk is
