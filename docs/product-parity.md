...
| Capability | Upstream behavior/reference | Current Desktop behavior | Classification | Priority | Confidence | Evidence | Owner / next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DTS Passthrough setting | Consumed by capability/audio policy | Hidden because native decode-to-system-output works without passthrough; passthrough is not a compatibility prerequisite. A default-off, per-format Settings feature is specified but deliberately deferred | PLANNED | P2 | HIGH | upstream settings/resolver; Desktop native options and architecture; `docs/audio-passthrough-spec.md` | Complete the physical Windows discovery gate, lock failure recovery, then implement the bounded typed setting and native contract |
