...
Production Unit 5D is the next authorized coding package, with its Windows
composition proof deferred to 5H. WS6–WS9 remain unopened.
...
- [ ] Execute the active nine-workstream parity-correction plan before RD-27:
      ...
      production Unit 5D is the next authorized coding package with Windows
      composition proof deferred to 5H;
...
- [x] Custom Channels Core completed on 2026-06-12 through the reviewed custom
  ...
  `expectedRevision`.
- [x] Complete WebOS UI Parity Reopen Packages 0–8 closed on 2026-07-16 with
  runtime-truth, focus, interaction, responsive, accessibility-mode, local
  fullscreen, and exact-capture proof. The integrated local result is 138
  screenshots split 69/69 across `1280x720` and `1920x1080`, plus 8
  reduced-motion, 12 forced-colors, and 3 local fullscreen rows. Package 6
  remains an explicit platform nonclaim: its three operator-assisted
  fullscreen focus rows must be run afresh on Windows in RD-27.
...
Feature parity and visual parity with upstream Lineup are both MVP acceptance
targets. A roadmap slice cannot close by replacing fake data behind the Desktop
scaffold while preserving a visibly scaffold-like user journey. When a slice
owns an upstream workflow, it must import or adapt the upstream screen
structure, visual hierarchy, interaction model, and product copy where those
fit Desktop boundaries, or record an explicit reviewed Desktop divergence with
the owner, reason, and follow-up trigger.
...
WS5 execution is locally complete through Unit 5C. The frozen plan checkpoint
...
the Settings-controlled, layout-specific Now Watching surface. Each checkpoint
typechecks independently; the focused Unit 5B suite passed 36/36, the Unit 5C
renderer suite passed 17/17, and cumulative `npm run verify` passed 1,209 of
1,210 contract tests with one intentional skip plus 177/177 harness/docs tests.
No Unit 5D product code is included in those checkpoints. Production Unit 5D
is the next authorized package; it uses the configured `worker` implementer
role because it crosses Electron view/window,
IPC/currentness, shared helper, Win32 presentation, DPI, and renderer seams.
Its wider Windows composition proof remains deferred to 5H and cannot be
claimed from this macOS closeout.
...
RD-01 through RD-26 historical slices are complete only at their recorded
scope, and Complete
WebOS UI Parity Reopen Packages 0–8 are locally complete and verified. Package
6's operator-assisted fullscreen platform proof remains bound to fresh RD-27
Windows execution. RD-21 superseded the former RD-21
...
Status: complete. RD-14 completed through the feature-quality loop on
2026-05-13. Unit 1 added a focused renderer desktop input owner for keyboard,
text-entry bypass, browser-safe gamepad normalization/polling/repeat policy,
fullscreen dispatch, and runtime cleanup while keeping renderer navigation and
focus unprivileged. Unit 2 moved BrowserWindow creation/options, fullscreen
intent execution, normal bounds capture, display id custody, and
restore/fallback placement policy into a focused main-owned window controller
while preserving the existing `window.setFullscreen(boolean)` response shape.
Unit 3 added a focused main-owned foreground app-command controller that uses no
`globalShortcut`, forwards only foreground `browser-backward` through synthetic
`Escape` input, intentionally ignores `browser-forward`, and leaves media
commands unhandled by product code. Unit 4 added renderer-owned DOM cursor
presentation over the fake-backed routes and overlays. Unit 5 closed the Windows
proof gate: RD-06 native-presentation preflight and smoke both passed on
Windows, producing ignored redacted evidence under
`docs/runs/rd-14-window-input-fullscreen-ux/`; the redacted Windows matrix
records window/fullscreen/restore, primary/secondary display presence,
multi-monitor availability, 100% DPI-only local coverage, focus/input over the
native-presentation surface and renderer overlays, native video surface proof,
cursor policy, media-key availability policy, local physical-gamepad hardware
unavailability plus Unit 1 automated browser-safe gamepad proof, text-input
unavailability in the current fake UI plus Unit 1 automated bypass proof, app
route/quit proof, helper cleanup, and forbidden-field checks. `npm run
smoke:electron`, `npm run verify:redaction`, `npm run
verify:docs`, and `npm run verify` passed on 2026-05-13. The completed full
plan body was archived locally under `docs/runs/archive/plans/` and removed
from tracked active plans. No upstream UI/source import, preload API,
renderer-facing privileged state, production native-helper playback, Plex
runtime behavior, dependency, package, or lockfile change landed in RD-14.
...
Status: historical slice complete and reviewed at its recorded scope. WS2
closes the current platform-neutral implementation gate; Windows/native/manual
proof remains `WS2-POST-VALIDATION-01` and RD-27 work as applicable.
...
- `RD-27 Windows proof required` for production
...
Status: historical slice complete and reviewed at its recorded scope. WS2
does not promote the conservative capability profile; Windows/native/manual
proof remains `WS2-POST-VALIDATION-01` and RD-27 work as applicable.

Depends on:

- RD-25 code complete and reviewed; local renderer parity complete; pending
  Windows/manual product proof belongs to RD-27.
...
- `RD-27 Windows proof required` for runtime media
...
- Runtime subtitle/audio/HDR code is covered by fixtures and policy tests for
  ...
  the completed local parity proof before Windows/manual product proof routes to
  RD-27.
...
WS5's plan is frozen at `73ce570`; Units 5A–5C land at `46acf1f`, `6180815`,
and `beeb5ef`, and production Unit 5D is next with Windows composition proof
...
- Upstream visual hierarchy, product copy, and interaction timing for each
  owned workflow. Adaptation is expected to preserve the product feel while
  honoring Desktop security, process, and accessibility boundaries.
