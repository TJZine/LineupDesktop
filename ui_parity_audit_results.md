...
**2026-07-10 reopen:** This audit is active input to the Tier 3 complete WebOS
UI parity reopen plan, not evidence of closeout. Exact current-reference and
Desktop-baseline captures live only in the ignored Package 0 bundle. RD-27 is
...
Desktop target: this repository
...
Desktop base tokens use smaller intermediate spacing and type than WebOS. Desktop has `--space-2: 6px`, `--space-4: 10px`, `--space-8: 18px`, `--button-min-height: 42px`, and base route/card styling in `src/renderer/styles/base.css:1`. WebOS uses a clearer 10-foot scale with `--text-base: 20px`, `--text-2xl: 36px`, `--space-2: 8px`, `--space-4: 16px`, `--space-8: 32px`, `--progress-height-lg: 10px`, and `--focus-ring-width: 3px` in `pinned upstream checkout src/styles/tokens.css:88`.
...
Desktop wraps most surfaces inside a topbar, route rail, bordered screen panel, and fixed route card layout in `src/renderer/styles/base.css:92` and `src/renderer/staticDom.ts:94`. WebOS settings, onboarding, EPG, and overlays are screen-owned immersive surfaces, not nested cards.

Recommendation: keep route ownership internally, but remove the permanent global route rail from product presentation. Each primary route must become a full-screen owner with its own explicit focus-entry and Back/shortcut contract; any recovery navigation must be invoked contextually rather than remain visible over player, guide, onboarding, settings, or playback overlay states.

### 3. Now Playing Is the Biggest Visual Parity Gap

WebOS builds a cinematic now-playing panel with backdrop, poster, clear logo, badges, playback summary, metadata, autoscrolling description, actor/cast regions, and progress meta in `pinned upstream checkout src/modules/ui/now-playing-info/NowPlayingInfoOverlay.ts:41` and `pinned upstream checkout src/modules/ui/now-playing-info/styles.core.css:22`. Desktop currently renders a small bottom-right text card with channel/title/subtitle/status and progress only in `src/renderer/staticDom.ts:13` and `src/renderer/styles/player-overlays.css:80`.

Recommendation: prioritize this after tokens. Keep the DOM/CSS artwork-ready, but render an honest artwork-empty state until renderer-safe artwork descriptors are available; do not fabricate placeholder art and do not expose tokenized Plex URLs.

### 4. OSD Is Structurally Functional But Not WebOS-Polished

WebOS OSD has a title/clear-logo area, direct Subtitles/Sleep/Audio actions, monospace tabular timecode, and a 10px progress bar in `pinned upstream checkout src/modules/ui/player-osd/PlayerOsdOverlay.ts:354` and `pinned upstream checkout src/modules/ui/player-osd/styles.meta-progress.css:1`. Desktop OSD has Mini guide, Options, number shortcuts, Close, audio/subtitle pills, and a 5px progress bar in `src/renderer/staticDom.ts:20` and `src/renderer/styles/player-overlays.css:120`.
...
WebOS mini guide is edge-attached at the top, five rows, with channel number, branding icon slot, channel name, now title/start time, progress, next title, loading shimmer, and a footer remote hint in `pinned upstream checkout src/modules/ui/mini-guide/MiniGuideOverlay.ts:162` and `pinned upstream checkout src/modules/ui/mini-guide/styles.core.css:5`. Desktop renders a centered modal-like card with previous/next buttons and article rows in `src/renderer/staticDom.ts:49` and `src/renderer/routeDom.ts:395`.
...
WebOS playback options open as a right-side rail and selected rows use an animated equalizer plus meta pills in `pinned upstream checkout src/modules/ui/playback-options/PlaybackOptionsModal.ts:130` and `pinned upstream checkout src/modules/ui/playback-options/styles.core.css:17`. Desktop uses a centered modal with summary cards, two columns, extra volume/rate controls, and no equalizer in `src/renderer/staticDom.ts:63` and `src/renderer/routeDom.ts:488`.
...
WebOS auth uses a QR card and per-character PIN boxes in `pinned upstream checkout src/modules/ui/auth/AuthScreen.ts:99` and `pinned upstream checkout src/styles/shell.onboarding.auth.css:12`. Desktop displays the PIN as a single `strong` inside the channel setup flow in `src/renderer/plexRuntimeDom.ts:85`.

WebOS profile PIN uses a modal with four PIN slots and a 0-9 on-screen numpad in `pinned upstream checkout src/modules/ui/profile-select/ProfileSelectScreen.ts:147` and `pinned upstream checkout src/modules/ui/profile-select/styles.pin-modal.css:1`. Desktop uses an inline numeric text input in `src/renderer/staticDom.ts:216`.
...
Desktop now renders live profile rows and server rows with ownership/connection/health summaries in `src/renderer/plexRuntimeRows.ts:32`. WebOS has richer status pills, auto-connect hint, glyph, and health/latency styling in `pinned upstream checkout src/modules/ui/server-select/ServerSelectScreen.ts:111` and `pinned upstream checkout src/modules/ui/server-select/styles.css:12`.
...
Desktop guide has improved since the earlier audit: it now has a classic header, now-watching banner, focus hints, info detail, and row/cell view model state in `src/renderer/routeDom.ts:207`. However, it still uses a fixed six-column CSS grid and `grid-column` spans in `src/renderer/routeDom.ts:297` and `src/renderer/styles/guide-epg.css:180`.

WebOS computes real pixel positions from schedule time and duration in `pinned upstream checkout src/modules/ui/epg/view/EPGProgramCellPosition.ts:4`, renders absolute cells with text shifting, compact tiers, episode badges, live badges, tickers, and progress in `pinned upstream checkout src/modules/ui/epg/view/cells/EPGCellRenderer.ts:52`, and styles library tabs, sticky header, edge masks, and a current-time indicator in `pinned upstream checkout src/modules/ui/epg/styles.grid.css:145` and `pinned upstream checkout src/modules/ui/epg/styles.cells.css:1`.
...
3. **No fake controls for missing features**
   - Sleep timer, latency badges, channel reordering, cast headshots, and clear-logo/artwork should appear only when backed by safe data. Otherwise render an honest empty state with no fabricated artwork or controls.
...
- Directly read WebOS UI files under `pinned upstream checkout src/modules/ui/**` and `pinned upstream checkout src/styles/**`.
